<!DOCTYPE html>
<body>

	<link rel="stylesheet" type="text/css" href="./css/6_sites/footer.css">
</body>
<html>
<script src="./js/main.js"></script>


<header>
	<h1>GermanFrenchFood, What is it ?</h1>
</header>


<p class="">We have two main idea that we had that pushed us to do this bi-national mix.
<h1>Germany</h1>For the challenge of developing their many applications on easy and quick to do dishes<h1>France</h1>was chosen to open up the complexity and freshness of their dishes.</p>

<p>Each of them can bring great change in the way we consume things, and is a great challenge to bring it over to you, in the best way possible. </p>


<ul>
	<h2>Guess what </h2>
	<p>Now the thing is that are you really gonna write or even read i'm actually typing right now ? 
	That is actually a very good question for you all ? do you know actually how many hours i've put into this typing thing ? </p>
	<h2>Also !</h2>
	<p>I'm quite proud of this little bad boy writing skills ! i've been practicing now for quite a few years and i'm actually very proud of it !</p>

	<h2>For The brave !</h2>
	<p>I'm actually surprised that you're still reading this ! so for recompensing the ones of you who made it that far, i'm gonna offer you all a cookie ! 
	the question is, will you be able to find it ? the cookie is hidden somewhere in the webpage (this one not another one don't worry about that.). if you can find it...</p>

	<h3>All is for you !</h3>
	<p>I mean the links to the websites, and everything else ! you just need to type the right things at the right moments ! and since i am in a good mood, this grandpa over there will give you a small hint. You just need to type only one word ;)</p>

	<h2>Trust me , you will find !</h2>

	<p>ok guys i'm becoming bored , so imma gonna stop right there, have some fun finding that stuff ! </p>
</ul>